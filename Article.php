...
class Article{
	...
	function show(){
		...
		if(isset($this->categoryName)){
			$join = mysqli_query($this->koneksi(), "SELECT article.*, user.username, user.first_name, user.sur_name, category.kategori FROM article INNER JOIN user ON article.userid = user.id INNER JOIN category ON article.kategoriID = category.id  WHERE category.kategori = '$this->categoryName' AND article.userid = '$this->userid'  ORDER BY article.dateUpdate DESC");
		}else{
			$join = mysqli_query($this->koneksi(), "SELECT article.id, article.userid, article.title, article.content, article.dateUpdate, user.username, user.first_name, user.sur_name FROM article INNER JOIN user ON article.userid=user.id WHERE article.userid = '$this->userid' ORDER BY article.dateUpdate DESC");
		}
					...
					<div class="col-md-12">
						<div class="row">
							...
							<div class="col-md-11">
								...
								oleh <a href="#" class="nama">'. $dataArtikel['first_name'] .' '.$dataArtikel['sur_name'].'</a>
							</div>
						</div>
						<div class="row">
	...
	function showList(){
		$i = 0;
		if(isset($this->categoryName)){
			$show = mysqli_query($this->koneksi(), "SELECT article.*, user.username, user.first_name, user.sur_name, category.kategori FROM article INNER JOIN user ON article.userid = user.id INNER JOIN category ON article.kategoriID = category.id  WHERE category.kategori = '$this->categoryName' ORDER BY article.dateUpdate DESC");
		}else{
			$show = mysqli_query($this->koneksi(), "SELECT article.id, article.userid, article.title, article.content, article.dateUpdate, user.username, user.first_name, user.sur_name FROM article INNER JOIN user ON article.userid=user.id ORDER BY article.dateUpdate DESC");
		}
